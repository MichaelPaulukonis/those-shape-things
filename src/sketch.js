...
const tileSequencer = () => {
  ...
  if (underlayIndex === 0) underlays = shuffle(underlays)
  return nextUnderlay
}
  ...
  for (let i = 0; i < cols; i++) {
    for (let j = 0; j < rows; j++) {
      ...
      if (random([true, false]))
        random(overlays)(x, y, tileSize, colors, random([true, false]))
    }
  }
}

function drawTileBase (x, y, tileSize, colors, drawShape, numShapes = null) {
  numShapes ??= int(random(2, 5))
...
function singleSquare (x, y, tileSize, colors) {
  blendMode(MULTIPLY)
  noFill()
  strokeWeight(random([1, 2, 3]))
  stroke(colors[0])
  rect(x + tileSize / 2, y + tileSize / 2, tileSize - 10, tileSize - 10)
  noStroke()
  blendMode(BLEND)
}

function doubleSquare (x, y, tileSize, colors) {
  const inset = random([1, 2, 3]) * 10
  blendMode(MULTIPLY)
  noFill()
  strokeWeight(random([1, 2, 3]))
  stroke(colors[0])
  rect(x + tileSize / 2, y + tileSize / 2, tileSize - inset, tileSize - inset)
  rect(x + tileSize / 2, y + tileSize / 2, tileSize - inset - 10, tileSize - inset - 10)
...
function drawSoloRing (x, y, tileSize, color, dashp, size, weight = null) {
  weight ??= random([2, 5, 10, 20])
  blendMode(MULTIPLY)
  noFill()
  stroke(color)
  strokeWeight(weight)
...
function doubleRings (x, y, tileSize, colors) {
  const weight = random([1, 2, 3])
  drawSoloRing(x, y, tileSize, colors[0], false, tileSize - 10, weight)
  drawSoloRing(x, y, tileSize, colors[0], false, tileSize - 20, weight)
}
...
function drawRings (x, y, tileSize, colors) {
  ...
  for (let i = numShapes; i > 0; i--) {
    ...
    drawSoloRing(x, y, tileSize, color, dashp, (tileSize * i) / numShapes)
  }
  blendMode(BLEND) // Reset blend mode to default
}
...
function drawRadialLinesTile (x, y, tileSize, colors) {
  ...
  drawSoloRing(x, y, tileSize, colors[0], random([true, true, false]), tileSize)
  drawSoloRing(
    x,
    y,
    tileSize,
